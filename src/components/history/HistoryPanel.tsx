...
export function HistoryPanel({ workflowId, open, onClose }: Props) {
  ...
  // SSE live update for any actively running run
  useEffect(() => {
    if (!open) {
    ...
    const sse = new EventSource(`/api/runs/${activeRun.id}/stream`);
    sseRef.current = sse;

    const handleUpdate = () => { fetchRuns(); };
    sse.addEventListener("node-start", handleUpdate);
    sse.addEventListener("node-finish", handleUpdate);
    sse.addEventListener("node-error", handleUpdate);
    sse.addEventListener("run-finish", () => {
      fetchRuns();
      sse.close();
      sseRef.current = null;
    });
  ...
  return (
    // Persistent flex sidebar — width transitions for smooth push
    <aside
      className={cn(
        "flex flex-col shrink-0 border-l border-white/5 bg-zinc-950/80 backdrop-blur-md shadow-2xl",
        "transition-[width] duration-300 ease-in-out overflow-hidden",
        open ? "w-[340px]" : "w-0"
      )}
    >
      {/* Inner container — fixed width so content doesn't wrap during transition */}
      <div className="w-[340px] flex flex-col h-full">
        ...
        <div className="h-14 px-4 flex items-center justify-between border-b border-white/5 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {hasActiveRun && (
              <span className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-semibold">
        ...
        <div className="px-4 py-2.5 flex items-center justify-between border-b border-white/5 shrink-0">
          ...
          <div className="relative">
            ...
            {filterOpen && (
              <div className="absolute right-0 top-full mt-1 w-36 bg-zinc-950/98 border border-white/10 rounded-xl shadow-2xl z-30 py-1 overflow-hidden backdrop-blur-md">
          ...
          {filteredRuns.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm text-zinc-500 text-center leading-relaxed">
                {statusFilter === "All" ? "No runs yet.\nClick Run to execute." : `No ${statusFilter.toLowerCase()} runs.`}
              </p>
            </div>
          ) : (
            filteredRuns.map((r) => {
              ...
              return (
                <div key={r.id} className="border-b border-white/5 last:border-0">
                  <button
                    onClick={() => toggle(r.id)}
                    className="w-full px-4 py-3 flex items-center gap-2.5 hover:bg-white/5 text-left transition-colors group"
                  >
                    {isExpanded ? (
                  ...
                  {/* Expandable node runs — smooth max-height transition */}
                  <div
                    className={cn(
