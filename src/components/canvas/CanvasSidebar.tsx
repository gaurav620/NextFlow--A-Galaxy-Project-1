...
const NAV_ITEMS = [
  ...
  { icon: LayoutGrid, label: "Library", href: "/dashboard" },
  { icon: GitBranch, label: "Flow", href: "/dashboard" },
  { icon: Cpu, label: "Nodes", href: "/dashboard" },
  { icon: Terminal, label: "API / MCP", href: "/dashboard" },
];
        ...
        <div className={cn("flex items-center h-12", collapsed ? "justify-center px-2" : "px-4 gap-2")}>
          {!collapsed && (
            <Link href="/dashboard" className="flex-1">
              <span className="text-lg font-bold tracking-tight text-neutral-900 dark:text-zinc-100 hover:opacity-80 transition-opacity" style={{ fontFamily: "'Georgia', serif" }}>
                NextFlow
              </span>
            </Link>
          )}
            ...
            {searchOpen && (
              <div className="px-1 pt-1">
                <input
                  ...
                  className="w-full px-2 py-1 text-[11px] rounded-md border border-neutral-200 dark:border-white/10 bg-neutral-50 dark:bg-zinc-900 text-neutral-900 dark:text-zinc-100 placeholder-neutral-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-purple-500/30"
                />
              </div>
            )}
          </div>
