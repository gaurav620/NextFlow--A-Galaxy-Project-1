...
export async function POST(req: Request) {
  try {
    ...
    // Create the run and node runs in DB
    ...
    // Start execution in background — dynamic import to avoid bundling issues
    // This runs AFTER the response is sent. On Vercel hobby, execution gets ~10s.
    // On Vercel Pro, maxDuration=300 allows up to 5 minutes.
    import("@/lib/execute").then(({ executeWorkflow }) => {
      executeWorkflow({
        runId: run.id,
        workflowId,
        graph: graph as Parameters<typeof import("@/lib/execute").executeWorkflow>[0]["graph"],
        scope,
        targetNodeIds,
        onEvent: () => {}, // events are persisted to DB inside executeWorkflow
      }).catch((err) => {
        console.error("[runs/route] execution failed:", err);
      });
    }).catch((err) => {
      console.error("[runs/route] failed to import execute:", err);
    });
